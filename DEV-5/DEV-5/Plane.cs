...
namespace DEV_5
{
    ...
    class Plane : IFlyable
    {
        ...
        public Plane(int x = 0, int y = 0, int z = 0) => CurrentPoint = new Point(x, y, z);

        /// <inheritdoc />
        public void FlyTo(Point newPoint)
        {
            Mileage += CurrentPoint.GetDistanceToPoint(newPoint);
            ObjectFlewAway?.Invoke(WhoAmI(), new ObjectFlewAwayEventArgs(GetFlyTime(), StartingSpeed + (int)Mileage));
